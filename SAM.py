...
def process_interior(image_rgb, output_path, sam_predictor):
    """Processes an interior image by whitening the windows using SAM."""
    print(f"Processing {os.path.basename(output_path)} as interior image...")

    h, w = image_rgb.shape[:2]

    # 1. Find seed points (brightest areas)
    gray = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2GRAY)
    # Blur to reduce noise
    gray_blurred = cv2.GaussianBlur(gray, (21, 21), 0)

    # Find the brightest 1% of pixels
    threshold_val = np.percentile(gray_blurred, 99)
    # Ensure threshold is at least high enough to be a window (e.g. > 200)
    threshold_val = max(threshold_val, 200)

    _, bright_mask = cv2.threshold(gray_blurred, threshold_val, 255, cv2.THRESH_BINARY)
    ...
    # Filter components
    min_area = (h * w) * 0.001  # Small threshold for seeds

    for i in range(1, num_labels):  # Skip background
        area = stats[i, cv2.CC_STAT_AREA]
        cx, cy = centroids[i]

        # Spatial constraint: Seed must be in top half
        if cy < h * 0.5 and area > min_area:
            points.append([cx, cy])
    ...
    if points:
        print(f"Found {len(points)} window seed points.")
        ...
        # Iterate over each point to get individual masks (handles separate windows)
        for i in range(len(points)):
            coords_torch = torch.as_tensor(point_coords_all[i:i + 1], dtype=torch.float, device=sam_predictor.device)
            labels_torch = torch.as_tensor(np.array([1]), dtype=torch.int, device=sam_predictor.device)

            # Add batch dimensions: (B, N, 2) -> (1, 1, 2)
        ...
        print("No bright window seeds found. Fallback to thresholding.")
        # Fallback: simple thresholding (stricter)
        hsv = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2HSV)
        lower_val = np.array([0, 0, 210])
        upper_val = np.array([180, 30, 255])
        final_mask = cv2.inRange(hsv, lower_val, upper_val)

    # Post-processing on the mask
    # Spatial constraint again (strictly enforce bottom 50% is clean)
    final_mask[int(h * 0.5):, :] = 0

    # Dilate slightly to ensure we cover the edges
